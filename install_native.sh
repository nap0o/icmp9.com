...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ -s /tmp/icmp9_old_endpoints.txt ]; then
        printf "已存接入点:       ${CYAN}列表如下:${NC}\n"
        while read -r line; do
            printf "                  ${CYAN}%s${NC}\n" "$line"
        done < /tmp/icmp9_old_endpoints.txt
    else
        printf "已存接入点:       ${YELLOW}未检测到地区配置${NC}\n"
    fi
...
ICMP9="/usr/bin/icmp9"
install_icmp9() {
    local url="https://o0o.net2ftp.pp.ua/https://github.com/nap0o/icmp9.com/releases/download/nginx/icmp9-native-${OS_TYPE}-${CF_ARCH}"
